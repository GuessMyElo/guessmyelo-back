    ...
    db.getConnection((err, connection) => {
      if (err) res.json({ error: true, err });
        ...
        connection.query("SELECT MAX(id) FROM users", (err, result) => {
          ...
          if (!err) {
            const maxId =
              result[0]["MAX(id)"] === null ? 0 : result[0]["MAX(id)"];
            res.json({ error: false, maxId });
            console.log("Success.");
          } else {
            res.json({
              error: true,
              message: "L'ID maximal n'a pas pu être récupéré.",
            });
            console.log(err.message);
          }
        });
                ...
                (err, result) => {
                  ...
                  if (!err) {
                    ...
                    res
                      .status(200)
                      .json({
                        tmessage: `L'utilisateur ${params.username} a été ajouté.`,
                        accessToken,
                        user: params,
                      });
                  } else {
        ...
        connection.query(
          ...
          (err) => {
            ...
            if (!err) {
              res.json({
                error: false,
                message: `L'utilisateur ${params.username} a été modifié.`,
              });
            } else {
              res.json({
                error: true,
                message: `L'utilisateur ${params.username} n'a pas été modifié.`,
              });
              console.log(err.message);
            }
          }
            ...
            if (!err) {
              if (result.length > 0) {
                bcrypt.compare(
                  ...
                  (err, response) => {
                    if (response) {
                      const { password, ...userResponse } = result[0];
                      const accessToken = jwt.sign(
                        userResponse,
                        process.env.JWT_SECRET
                      );
